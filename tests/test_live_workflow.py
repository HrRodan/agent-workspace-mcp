...
DEFAULT_MODEL = os.environ.get("DEFAULT_MODEL", "openrouter/google/gemini-3-flash-preview")
...
class MCPContainerClient:
    ...
    async def run_tool(self, name: str, arguments: dict):
        ...
        res = await self.call("tools/call", {
            "name": name,
            "arguments": arguments
        })
...
"""
        await client.run_tool("write_file", {"filepath": "hello.py", "content": script_content})
...
async def test_workflow_project_bootstrap(tmp_path):
    ...
    try:
        ...
        # 1. uv init
        await client.run_tool("run_bash", {"command": "uv init --name myproject --no-readme"})
        
        # 2. Update hello.py
        await client.run_tool("write_file", {"filepath": "hello.py", "content": "print('bootstrapped')"})
