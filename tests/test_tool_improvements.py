import pytest
from agent_workspace_mcp.tools.filesystem import read_file, list_directory, search_workspace
...
async def test_live_tool_workflow(workspace, mock_ctx):
    ...
    search_res = await search_workspace("**/*.py", exclude_patterns=["src/main.py"], ctx=mock_ctx)
